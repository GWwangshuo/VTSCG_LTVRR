...
class MaxNorm_via_PGD():
    ...
    def setPerLayerThresh(self, model):
        ...
        for curLayer in [model.interaction_head.box_pair_predictor.weight, model.interaction_head.box_pair_predictor.bias, 
                         model.interaction_head.box_h_predictor.weight, model.interaction_head.box_h_predictor.bias]: #here we only apply MaxNorm over the last two layers
    ...
    def PGD(self, model):
        ...
        for i, curLayer in enumerate([model.interaction_head.box_pair_predictor.weight, model.interaction_head.box_pair_predictor.bias, 
                         model.interaction_head.box_h_predictor.weight, model.interaction_head.box_h_predictor.bias]): #here we only apply MaxNorm over the last two layers
            curparam = curLayer.data
...
class Normalizer(): 
    ...
    def apply_on(self, model): #this method applies tau-normalization on the classifier layer
        ...
        for curLayer in [model.interaction_head.box_pair_predictor.weight, model.interaction_head.box_h_predictor.weight]: #change to last layer: Done
            curparam = curLayer.data
